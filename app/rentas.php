				...
				<table>
					<tr>
						...
						<?php $query = $db->query("SELECT Prestamos.*, Contenido.Nombre
							...
							WHERE Usuarios_idUsuario = '".$_SESSION['uid']."'
								AND FechaDevuelto IS NULL");
					...
					<tr>
						...
						<?php $query = $db->query("SELECT Prestamos.*, Contenido.Nombre
							FROM Prestamos 
								...
								JOIN Contenido ON Prestamos_has_Copia.Copia_Contenido_idContenido = Contenido.idContenido
							WHERE Usuarios_idUsuario = '".$_SESSION['uid']."'
								AND FechaDevuelto IS NOT NULL");
						while($res = $query->fetch_object()){ ?>
					<tr>
						<td><?=$res->Nombre;?></td>
						<td><?=(new DateTime($res->FechaPrestamo))->format('d/m/y');?></td>
						<td><?=(new DateTime($res->FechaDevuelto))->format('d/m/y');?></td>
					</tr>
				<?php } ?>
				</table>
			</div>
